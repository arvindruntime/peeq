...
(function ($) {
  var office_base_js = {
    ...
    initialize_ele: function initialize_ele() {
      ...
      office_base_js.tooltip(); // office_base_js.quiz_hide();
      // office_base_js.emoji_on();
    },
    light_dark: function light_dark() {
      $(function () {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var calendar = new FullCalendar.Calendar(calendarEl, {
    headerToolbar: {
      ...
      right: 'listMonth,timeGridDay,timeGridWeek,dayGridMonth' // listMonth,timeGridDay,timeGridWeek,dayGridMonth
...
function formatState(state) {
  ...
  var $state = $('<div class="d-flex gap-2 align-items-center"> <span class="avtar-30"><img src="' + $(state.element).attr('data-src') + '" class="img-flag" /></span> ' + state.text + '</span></div>');
  return $state;
}
