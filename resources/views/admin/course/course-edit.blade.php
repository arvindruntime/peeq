                ...
                <div class="row">
                  <div class="col-12">
                    <div class="mb-3 admin-file-upd">
                      <label class="form-label w-100" for="course_thumbnail">
                        ...
                        <div class="admin-file-upd-input position-relative mt-1 thunbnail-edit">
                          <label class="form-label w-100" for="course_thumbnail_change_image">
                            <input class="d-none" type="file" name="course_thumbnail" id="course_thumbnail_change_image">
                      ...
                      {{-- <input class="form-control" name="course_thumbnail" id="course_thumbnail" type="hidden" > --}}
                    </div>
                    <div class="mb-3 admin-file-upd">
                        ...
                        <div class="admin-file-upd-input position-relative mt-1 thunbnail-edit">
                          <label class="form-label w-100" for="course_vedio_change">
                            <div class="lm_vedio-card card">
                              <input class="d-none" type="file" name="course_preview_video" id="course_vedio_change">
                              <div class="card-img position-relative">
                                  <video controlsList="nodownload" controls class="w-100  course_preview_video" name="course_preview_video" src="{{ $course['course_preview_video'] ?? '' }}">
                                    <source src="{{ $course['course_preview_video'] ?? '' }}" type="video/mp4">
                                    <source src="{{ $course['course_preview_video'] ?? '' }}" type="video/ogg">
                                  </video>
                              </div>
                            </div>
                    ...
                    <div class="mb-3 admin-file-inp"><label class="form-label" for="course_name">Course Name</label>
                      <input class="form-control shadow" name="course_name" id="course_name" type="text" placeholder="Course Name" value="{{ $course['course_name'] ?? '' }}">
                    </div>
                    <div class="mb-3 admin-file-inp"><label class="form-label" for="course_tagline">Course Tagline (Optional)</label>
                      <input class="form-control shadow" name="course_tagline" id="course_tagline" type="text" placeholder="Course Tagline" value="{{ $course['course_tagline'] ?? '' }}">
                    </div>
                    <div class="mb-3 admin-file-select">
                      <label for="id_label_multiple">Add Coaches</label>
                      @php $course_price = '';
                        $selectedCoaches = explode(",",$course['coaches_string']);
                        if($course['course_price_type'] == "paid")
                        {
                          $course_price = $course['course_price'];
                        }
                      @endphp
                      <select name="coaches[]" id="coaches" class="select2 form-select js-example-templating js-states form-control select2-hidden-accessible select-img" aria-label="Default select example" multiple="multiple">
                        @foreach (coachList() as $coach)
                            <option value="{{ $coach->id }}" data-src="{{ $coach->profile_image_url }}"
                                @if (in_array($coach->id, $selectedCoaches)) selected @endif
                    ...
                    <div class="mb-3 admin-file-textarea">
                      ...
                      <textarea class="form-control shadow" name="description" id="description" rows="3" placeholder="Description">{{  $course['description'] ?? ''  }}</textarea>
                    </div>
                    
                    <div class="mb-3 admin-file-textarea">
                      <label class="form-label" for="description">Module Overview Description</label>
                      <textarea class="form-control shadow" name="module_overview_description" id="module_overview_description" rows="3" placeholder="Module Overview Description...">{{  $course['module_overview_description'] ?? ''  }}</textarea>
                    </div>
                    
                    <div class="mb-3 admin-file-textarea">
                      <label class="form-label" for="upload_pdf">Upload PDF</label>
                      @if(!empty($course['upload_pdf']))
                          (<a href="{{ old('upload_pdf', $course['upload_pdf']) }}" target="_blank">One file attached</a>)
                      @else
                          (No file attached)
                      @endif
                      <input class="form-control shadow" name="upload_pdf" id="upload_pdf" type="file" value="{{ $course['upload_pdf'] ?? '' }}">
                    </div>

                    <div class="mb-3 admin-file-upd">
                      <label class="form-label w-100" for="course_completed_image">
                        Course Completed Image
                        <div class="admin-file-upd-input position-relative mt-1">
                            <div class="admin-file-upd-input position-relative mt-1 thunbnail-edit " id="thumnail_coure_complate_img">
                              <label class="form-label w-100" for="course_completed_thumbnail_change_image">
                                <input class="d-none" type="file" name="course_completed_image" id="course_completed_thumbnail_change_image">
                                 <img class="rounded-4 course_completed_thumbnail_image_preview " name="course_completed_image"
                                      src="{{ $course['course_completed_image'] ?? '' }}" alt="">
                                <a class="btn btn--dark " id="change_completed_image_hide">Change Image</a>
                              <label>
                            </div>
                        </div>
                      </label>
                    ...
                    <div class="mb-3 admin-file-inp">
                      ...
                      <input class="form-control shadow" name="google_pay_id" id="google_pay_id"  type="text" placeholder="Google pay id" value="{{ $course['google_pay_id'] ?? '' }}">
                    </div>
                    
                    <div class="mb-3 admin-file-inp">
                      <label class="form-label" for="apple_pay_id">Apple pay id (Optional)</label>
                      <input class="form-control shadow" name="apple_pay_id" id="apple_pay_id" type="text" placeholder="Apple pay id" value="{{ $course['apple_pay_id'] ?? '' }}">
                    </div>
                    
                    <div class="mb-3 admin-file-inp">
                      <label class="form-label" for="stripe_subscription_course_id">Stripe subscription course id (Optional)</label>
                      <input class="form-control shadow" name="stripe_subscription_course_id" id="stripe_subscription_course_id" type="text" placeholder="Stripe subscription course id" value="{{ $course['stripe_subscription_course_id'] ?? '' }}">
                    </div>
                    ...
                    <div class="course-price-tab my-2">
                      <ul class="nav nav-pills mb-3 nav-primary shadow" id="pills-tab" role="tablist">
                        <li class="nav-item" role="presentation">
                          <button class="nav-link {{ ($course['course_price_type'] == "free") ? 'active' : '' }}" onclick="setPriceType('free')" id="pills-free-tab" data-bs-toggle="pill" data-bs-target="#pills-free" type="button" role="tab"
                          aria-controls="pills-free" aria-selected="true">Free</button>
                        </li>
                        <li class="nav-item" role="presentation">
                          <button class="nav-link {{ ($course['course_price_type'] == "paid") ? 'active' : '' }}" onclick="setPriceType('paid')" id="pills-paid-tab" data-bs-toggle="pill" data-bs-target="#pills-paid" type="button" role="tab"
                      ...
                      <div class="tab-content" id="pills-tabContent">
                        ...
                        <div class="tab-pane fade {{ ($course['course_price_type'] == "paid") ? 'active show' : '' }}" id="pills-paid" role="tabpanel"
                          aria-labelledby="pills-pais-tab" tabindex="0">
                          <div class="row">
                            <div class="col-md-6 mb-3">
                              <div class="admin-file-inp">
                                <input class="form-control shadow" name="course_price_type" id="course_price_type" type="hidden" value="{{ $course['course_price_type'] ?? 'free' }}">
                                <input class="form-control shadow" name="course_price" id="course_price" type="number" placeholder="e.g. 499" value="{{ $course_price  }}">
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div class="lm_crt-btn d-flex justify-content-center align-item-center">
                        <button class="btn btn--primary" id="SaveCourseBtn" type="button">Update</button>
